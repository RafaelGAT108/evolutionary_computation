from libs.clonalg import ClonalG
...
if __name__ == "__main__":
    ...
    final = time.time()
    print("-"*100)
    print(f"TEMPO DE 30 EXECUÇÕES DO GA: {round(final - initial, 2)} seg")
    print("-" * 100)
    ...
    initial = time.time()
    for i in range(30):
        pso = ParticleSwarmOptimization(cost_function=levy_cost,
                                        interations_size=30,
                                        size_particles=440,
                                        c1=2, c2=2,
                                        with_update_w=True,
    ...
    final = time.time()
    print("-" * 100)
    print(f"TEMPO DE 30 EXECUÇÕES DO PSO: {round(final - initial, 2)} seg")
    print("-" * 100)
    ...
    final = time.time()
    print("-" * 100)
    print(f"TEMPO DE 30 EXECUÇÕES DO DE: {round(final - initial, 2)} seg")
    print("-" * 100)
    ...
    best_fitness_clonalg = []
    mean_fitness_clonalg = []

    initial = time.time()
    for i in range(30):
        clonalg = ClonalG(population_size=440,
                          cloning_rate=0.5,
                          mutation_range=0.9,
                          interations_size=30,
                          selection_rate=0.9,
        ...
        best_fitness_clonalg.append(best_fitness)
        mean_fitness_clonalg.append(mean_fitness)
    final = time.time()
    print("-" * 100)
    print(f"TEMPO DE 30 EXECUÇÕES DO CLONALG: {round(final - initial, 2)} seg")
    print("-" * 100)

    boxplot_data = np.hstack((boxplot_data, np.array(best_fitness_clonalg).reshape(-1, 1)))
    boxplot_data_mean = np.hstack((boxplot_data_mean, np.array(mean_fitness_clonalg).reshape(-1, 1)))

    plt.figure(figsize=(10, 5))
    plt.title(f"Mean Fitness (levy_cost)")
    df2 = pd.DataFrame(boxplot_data_mean, columns=['GA', 'PSO', 'DE', 'CLONALG'])
    boxplot2 = df2.boxplot()
    fig2 = boxplot2.get_figure()

    fig2.savefig('boxplot_mean_levy_cost_comparative_ga_pso_de_clonalg.png')

    plt.figure(figsize=(10, 5))
    plt.title(f"Best Fitness (levy_cost)")
    df = pd.DataFrame(boxplot_data, columns=['GA', 'PSO', 'DE', 'CLONALG'])
    boxplot = df.boxplot()
    fig = boxplot.get_figure()

    fig.savefig('boxplot_best_levy_cost_comparative_ga_pso_de_clonalg.png')
